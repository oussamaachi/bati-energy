...
export default function Navbar() {
    ...
    const navClass = `fixed top-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 rounded-full pl-6 pr-4 py-3 xl:pr-3 flex items-center justify-between gap-4 xl:gap-8 min-w-[300px] w-max max-w-[96vw] ${scrolled || !isHome
    ...
    return (
        <nav className={navClass}>
            <Link to="/" className="flex items-center gap-2 hover-link group shrink-0 h-8 w-32 overflow-hidden relative justify-center">
                <img src="/logo.png" alt="BATI ENERGY" className={`absolute w-full h-full object-contain scale-[2] ${!scrolled && isHome ? 'brightness-0 invert' : ''}`} />
            </Link>
